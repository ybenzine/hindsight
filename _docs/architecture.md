...
| Live updates | Server-Sent Events (SSE) | One-way push for reveal, votes, mode changes, job status. No WebSockets, no Redis channel layer required for a single worker |
...
| Background jobs | Celery + Redis broker | Transcription and extraction are minutes-long; must be off the request path |
| Object storage | S3-compatible via `django-storages` | Local filesystem in dev; bucket in prod. Cloud-agnostic |
| Transcription provider | **Deferred** | Isolated behind a `Transcriber` interface (see [AI integration](#ai-integration)) |
...
| Hosting | **Deferred** | Build cloud-agnostic: Postgres + S3-compatible storage + Redis + one web process + one worker process |

### Why SSE over Channels for the MVP

The plan needs *broadcast* ("everyone sees the reveal", "vote totals appear when voting closes", "job finished"), not bidirectional low-latency sync. SSE over a plain HTTP endpoint covers all of it, runs on a normal WSGI/ASGI setup, and needs no Redis for pub/sub when there is a single web process. If we later scale to multiple web processes, the SSE view switches its event source from an in-process queue to Redis pub/sub — the client contract does not change.
...
MeetingRecord     (Retrospective, upload_kind: audio|video|transcript_file|pasted_text,
                   file, raw_text, transcript_text, status: pending|transcribing|extracting|ready|failed)
...
### Cluster
Clustering job (Celery + Claude) creates draft `Cluster` + `ClusterCard` rows. Drag/merge/split/rename hit minimal JSON endpoints; each mutation broadcasts `board.changed` with the affected cluster id(s); other clients `hx-get` the fragment.
...
`POST` to `meetings/` saves the `MeetingRecord` (file to storage or pasted text) → Celery chain:
1. `transcribe` (skipped if transcript/text provided) → `Transcriber.transcribe(file) -> transcript_text`
...
### SSE endpoint
`GET /events/retro/<id>` — long-lived `text/event-stream`. Auth-checked, membership-checked. Emits JSON `{type, ...ids}`. Client (Alpine) maps `type` → an `hx-get` on the relevant fragment. Heartbeat comment every ~20s. In-process `asyncio.Queue` / simple pub-sub now; Redis pub-sub swap-in later.

## AI integration

All model calls live in `apps/ai/` and are called only from Celery tasks, never inline in a view.

- **`Transcriber` protocol** — `transcribe(source: FileField | str) -> str`. Implementations: `NullTranscriber` (dev, echoes pasted text / raises for media), and a real provider added later. Provider choice is deferred; nothing else in the codebase imports a provider SDK directly.
- **Clustering** — `suggest_clusters(cards) -> list[{title, card_ids}]`. Claude, single call, temperature low, JSON output validated against a schema. Output is *draft* rows the team edits.
...
- Costs/latency: both calls are async, retried by Celery with backoff, and their status surfaces to the user via SSE.
...
`django-environ` reads: `DATABASE_URL`, `REDIS_URL`, `SECRET_KEY`, `DJANGO_SETTINGS_MODULE`, `STORAGE_BACKEND` (`filesystem` | `s3`), `AWS_*` / S3-compatible creds, `ANTHROPIC_API_KEY`, `TRANSCRIBER` (`null` | provider), email backend vars.

Dev: SQLite is *not* used (JSONB + Postgres-specific behavior) — local Postgres via `docker-compose` (Postgres + Redis). Media to local `./media/`.

## Deployment shape (host chosen later)

Cloud-agnostic target = 4 things any host can provide:

1. **web** process — `gunicorn`/`uvicorn` running the Django app
2. **worker** process — `celery worker`
3. **Postgres** (managed)
4. **Redis** (Celery broker; later also SSE fan-out)
5. **S3-compatible bucket** for uploads

`beat` (scheduled tasks) only if we add cycle auto-open later; not in MVP.

## Build sequence

1. Project skeleton, settings split, docker-compose (Postgres + Redis), base templates + HTMX/Alpine, auth (allauth), `common` base models.
...
8. `ai` clustering: Celery task + Claude `suggest_clusters`, wired into step 5 as draft rows.
9. `meetings`: upload/paste, `MeetingRecord`, Celery chain with `NullTranscriber`, `ExtractionDraft`.
...
13. Pick transcription provider, implement real `Transcriber`, swap `TRANSCRIBER` env.
14. Pick host, add process config + storage/env wiring, first deploy.
